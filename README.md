...
Select the **brainstormer** agent, then describe your idea:
...
Optionally configure agent settings in `~/.config/opencode/brainstormer.json`:
...
Instead of linear questioning, brainstormer creates **parallel exploration branches**. Each branch has a scope (e.g., "Architecture", "Data Model", "Security") and asks questions within that scope.
...
|---------|---------|---------|
| **State Session** | Branch state, Q&A history, findings | `.brainstorm/{id}.json` |
...
| Agent | Role |
|-------|------|
| `brainstormer` | Orchestrator - coordinates flow |
...
├── agents/
│   ├── brainstormer.ts   # Orchestrator prompt
